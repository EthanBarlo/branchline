  ...
  return <>
    <div className="update-details">
      ...
      <div className="update-status" role="status" aria-live="polite">
        ...
        <div><h3>{title}</h3><p>{phase === 'disabled' ? state.disabledReason : phase === 'downloaded' ? 'Restart to install. Your review work will be saved first. macOS may ask for an administrator password.' : phase === 'downloading' ? 'You can keep reviewing while the download finishes.' : 'Updates are checked automatically. You choose when to download and restart.'}</p></div>
      </div>
